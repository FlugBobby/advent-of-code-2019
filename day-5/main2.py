...
input_test_3 = "3,9,8,9,10,9,4,9,99,-1,8"
input_test_4 = "3,9,7,9,10,9,4,9,99,-1,8"
input_test_5 = "3,3,1108,-1,8,3,4,3,99"
input_test_6 = "3,3,1107,-1,8,3,4,3,99"

input_test_7 = "3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9"
input_test_8 = "3,3,1105,-1,9,1101,0,0,12,4,12,99,1"

input_test_9 = "3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99"
...
def jump_if_true(input_array, modes, args, current_position):
    if (get_value_mode(input_array, modes, args, 0) != 0):
        return (get_value_mode(input_array, modes, args, 1))
    return (current_position + 3)

def jump_if_false(input_array, modes, args, current_position):
    if (get_value_mode(input_array, modes, args, 0) == 0):
        return (get_value_mode(input_array, modes, args, 1))
    return (current_position + 3)

def less_than(input_array, modes, args, current_position):
    if (get_value_mode(input_array, modes, args, 0) < get_value_mode(input_array, modes, args, 1)):
        store_result(input_array, modes, args, 2, current_position, 1)
    else:
        store_result(input_array, modes, args, 2, current_position, 0)
    return (current_position + 4)

def equals(input_array, modes, args, current_position):
    print (get_value_mode(input_array, modes, args, 0))
    
    if (get_value_mode(input_array, modes, args, 0) == get_value_mode(input_array, modes, args, 1)):
#        print("equals?: TRUE", get_value_mode(input_array, modes, args, 0), get_value_mode(input_array, modes, args, 1))
        store_result(input_array, modes, args, 2, current_position, 1)
    else:
#        print("equals?: false", get_value_mode(input_array, modes, args, 0), get_value_mode(input_array, modes, args, 1))
        store_result(input_array, modes, args, 2, current_position, 0)
    return (current_position + 4)
...
def treat_opcode(input_array, i):
    ...
    return (operations[opcode][0](input_array, modes, args, i))
#    return (i + args_nbr)

def run_machine(_input):
